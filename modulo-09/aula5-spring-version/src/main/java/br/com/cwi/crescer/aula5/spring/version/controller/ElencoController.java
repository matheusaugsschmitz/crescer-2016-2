...
public class ElencoController {
    ...
    public String list(Model model, @RequestParam(required = false) Long id, Pageable p) {
        ...
        if (id != null) {
            elenco = elencoService.findOne(id);
            pageable = null;
        }
        if (pageable == null) {
        ...
        model.addAttribute("elencos", elencoService.findAll(pageable));
        model.addAttribute("atoresAll", atorService.list());
        return "elenco";
    }
